...
int EngineManager::Destroy()
{
	...
	while (1){
	DWORD dwExitCode = NULL;
	GetExitCodeThread(mWarpingMapThread, &dwExitCode);
	if (dwExitCode != STILL_ACTIVE)
	break;
	Sleep(WAIT_WARP_TIME);

	printf("bigheadk, wait 3 \n");
	}
	*/
...
unsigned char* EngineManager::setSelectPosition(int i, int bound_x, int bound_y, int bound_w, int bound_h)
{
	...
	if (nCount != 0)
	{
		...
		//printf("[%d] CAL center (%f,%f), sum (%d,%d), nCount : %d\n", i, mCenter_x[i], mCenter_y[i], nSumX, nSumY, nCount);
	}
	else
	{
	...
	if (manager->stitchingMode != STITCHED_INPUT)
	{
		...
		if (channel_idx == 1)
		{
			...
			if (manager->mCaptureDoneCounter == manager->mNumCamera)
			{
				...
				else if (manager->stitchingMode == 0) {
					if (manager->mIsCalibrationThread){
						manager->mIsCalibrationThread = false;
						manager->mCalibrationThread = CreateThread(NULL, 0, run_quram_calib, manager, 0, NULL);
					}
				}
				else if (manager->stitchingMode == WIDE_STITCHING) {
					if (manager->mIsCalibrationThread){
						manager->mIsCalibrationThread = false;
						manager->mCalibrationThread = CreateThread(NULL, 0, run_quram_calib, manager, 0, NULL);
					}
