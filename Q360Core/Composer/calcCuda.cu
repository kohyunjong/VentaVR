...
__global__ void convertRGBtoYUV420p(unsigned char *input, unsigned char *output, int dst_width, int dst_height);
...
void cu_RGBtoYUV444(unsigned char *yuv, unsigned char *pData, int width, int height)
{
	...
	convertRGBtoYUV420p << <dimgrid, dimblock >> >(pData, yuv, width, height);
}
