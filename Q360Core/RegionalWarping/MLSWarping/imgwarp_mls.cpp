...
ImgWarp_MLS::ImgWarp_MLS() { gridSize = 1; }
...
Mat ImgWarp_MLS::setAllAndGenerate(const Mat &oriImg,
	...
	fprintf(stdout, "setAllAndGenerate...\n");

	fprintf(stdout, "src pts : ");
	for (int i = 0; i < qsrc.size(); i++)
		fprintf(stdout, "(%d %d), ", qsrc.at(i).x, qsrc.at(i).y);
	fprintf(stdout, "\ndst pts : ");
	for (int i = 0; i < qdst.size(); i++)
		fprintf(stdout, "(%d %d), ", qdst.at(i).x, qdst.at(i).y);
	fprintf(stdout, "\n");
	...
	*result_mapx = rDx.clone();
	*result_mapy = rDy.clone();

    return genNewImg(oriImg, transRatio);
}

void ImgWarp_MLS::setAllAndGenerate(int w, int h,
	const vector<Point_<int> > &qsrc,
	const vector<Point_<int> > &qdst,
	const int outW, const int outH,
	const double transRatio,
