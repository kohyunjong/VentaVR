...
{
	//Check if pic data is loaded //여기서 더블 버퍼링을 써야 할듯..
	if(m_bIsPicLoaded)
	{
		...
		bmp.CreateCompatibleBitmap(CDC::FromHandle(lpDrawItemStruct->hDC), rc.Width(), rc.Height()); //도화지 준비!
		...
		BitBlt(lpDrawItemStruct->hDC, 0, 0, rc.right, rc.bottom, MemDC, 0, 0, SRCCOPY);
