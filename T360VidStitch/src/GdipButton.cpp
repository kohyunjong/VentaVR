...
CGdipButton::~CGdipButton()
{
	...
	if (m_pOnOverImage) delete m_pOnOverImage;
	if (m_pOnPressImage) delete m_pOnPressImage;
...
BOOL CGdipButton::LoadOvrImage(UINT id, LPCTSTR pType)
{
	m_pOverImage = new CGdiPlusBitmapResource;
	return m_pOverImage->Load(id, pType);
}

BOOL CGdipButton::LoadPressImage(UINT id, LPCTSTR pType)
{
	m_pPressImage = new CGdiPlusBitmapResource;
	return m_pPressImage->Load(id, pType);
}

BOOL CGdipButton::LoadOnOvrImage(UINT id, LPCTSTR pType)
{
	m_pOnOverImage = new CGdiPlusBitmapResource;
	return m_pOnOverImage->Load(id, pType);
}

BOOL CGdipButton::LoadOnPressImage(UINT id, LPCTSTR pType)
{
	m_pOnPressImage = new CGdiPlusBitmapResource;
	return m_pOnPressImage->Load(id, pType);
}

BOOL CGdipButton::LoadImages(UINT stdid, UINT altid, UINT ovrid, UINT prsid, UINT onovrid, UINT onprsid, LPCTSTR pType)
{
	BOOL bRtn = FALSE;
	...
	if (ovrid != 0)
		bRtn = LoadOnOvrImage(onovrid, pType);
	if (prsid != 0)
		bRtn = LoadOnPressImage(onprsid, pType);
	...
	if(!m_bHaveBitmaps)
	{
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			if(m_dcGS.m_hDC == NULL)
			{
				...
				ColorMatrix GrayMat = { 0.23f, 0.23f, 0.23f, 0.00f, 0.00f,
										0.30f, 0.30f, 0.30f, 0.00f, 0.00f,
										0.23f, 0.23f, 0.23f, 0.00f, 0.00f,
										0.00f, 0.00f, 0.00f, 1.00f, 0.00f,
		...
		if( (m_dcAlt.m_hDC == NULL) && m_bHaveAltImage )
		{
			...
			if( (m_dcAltP.m_hDC == NULL) && m_bHaveAltImage )
			{
				...
				if (m_pOnPressImage)
					graphics.DrawImage(*m_pOnPressImage, 1, 1);
				else
			...
			if (m_dcAltH.m_hDC == NULL)
			{
				...
				RectF grect; grect.X=0, grect.Y=0; grect.Width = width; grect.Height = height;
				if (m_pOnOverImage != NULL)
					graphics.DrawImage(*m_pOnOverImage, grect, 0, 0, width, height, UnitPixel);
				else
...
//=============================================================================
// sets the color and disabled state or enable state then repaints
//=============================================================================

void CGdipButton::Disable(CGdipButton* btn)
{
	btn->EnableWindow(FALSE);
	CGdipButton::SetImage(CGdipButton::DIS_TYPE);
}

void CGdipButton::Enable(CGdipButton* btn)
{
	btn->EnableWindow(TRUE);
	CGdipButton::SetImage(CGdipButton::STD_TYPE);
}
...
void CGdipButton::SizeToContent()
{
	CRect rect;
	GetClientRect(&rect);
	if (m_pStdImage)
...
void CGdipButton::SetButtonPos(int x, int y)
{
	CRect rect;
	GetWindowRect(&rect);
	if (m_pStdImage)
	{
		float width = (float)m_pStdImage->m_pBitmap->GetWidth();
		float height = (float)m_pStdImage->m_pBitmap->GetHeight();
		SetWindowPos(NULL, x, y, width, height, SWP_SHOWWINDOW);
	}
}