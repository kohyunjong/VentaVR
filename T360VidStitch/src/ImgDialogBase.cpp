...
CMap<HWND, HWND, LPCtrlInfo, LPCtrlInfo>	CImgDialogBase::s_mpCtrl;
int CImgDialogBase::m_nTickCount = GetTickCount();
//-------------------------------------------------------------------------
	...
	, m_bIsRefreshing(FALSE)
{
	...
	m_pImage = LoadImage(nImgID, lpszType, hResourceModule);
	...
	m_strWndClassName.Format( _T("FakeWnd%X"), m_nTickCount);
}
...
void CImgDialogBase::HookControlUpdate(HWND hWnd)
{
	...
	s_mpCtrl[hWnd] = new tagCtrlInfo(stCtrlInfo);
...
// 		s_mpCtrl.erase(iter);
	...
	if (s_mpCtrl.Lookup(hWnd, prTemp))
		s_mpCtrl.RemoveKey(hWnd);
...
// 		return ::DefWindowProc(hWnd, nMsg, wParam, lParam);
	...
	if (!s_mpCtrl.Lookup(hWnd, iter))
		return ::DefWindowProc(hWnd, nMsg, wParam, lParam);

	//LRESULT lResult = iter->second.pWndProc(hWnd, nMsg, wParam, lParam);
	LRESULT lResult = iter->pWndProc(hWnd, nMsg, wParam, lParam);
	switch (nMsg)
	{
	case WM_PAINT:
...
void CImgDialogBase::OnShowWindow(BOOL bShow, UINT nStatus)
{
	...
	{
		m_bShown = TRUE;
		//AfxBeginThread((AFX_THREADPROC)ShowMotionThread, this); //KBJ
	}
}
...
Image * CImgDialogBase::LoadImage(UINT nID, LPCTSTR lpszType, HINSTANCE hInstance /*=NULL*/)
{
	Image * pImage = NULL;
