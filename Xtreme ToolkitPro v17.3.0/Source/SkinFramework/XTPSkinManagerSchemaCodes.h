...
public:
	static int MapKey(const TMS_SCHEMA_ENTRY *pArray, LPCTSTR key, LPCTSTR prefix = NULL)
	{
		ASSERT(NULL != pArray);
		ASSERT(NULL != key);
		...
		if (!Instance().m_cache.Lookup(szHashName, nValue))
		{
			int i = 0;
	...
	static int MapKey2(const TMS_SCHEMA_ENTRY *pArray, LPCTSTR key, LPCTSTR key2)
	{
		ASSERT(NULL != key);
		ASSERT(NULL != key2);
