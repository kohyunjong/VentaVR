...
public:
	...
	//-----------------------------------------------------------------------
	static void Copy(const CXTPTagCloudStyle* pSource, CXTPTagCloudStyle* pDest);

	int      m_nScore;       // Unique identifier used to specify the score or weight of the style.
	CFont    m_font;         // Font used by the style.
...
private:
	...
	static int CompareName(XTP_TAGCLOUDTAG* pTagA, XTP_TAGCLOUDTAG* pTagB, BOOL bAsc);
	static int CompareScore(XTP_TAGCLOUDTAG* pTagA, XTP_TAGCLOUDTAG* pTagB, BOOL bAsc);
