...
protected:
	...
	//-----------------------------------------------------------------------
	CXTPObservable() : m_nNotificationDepth(0)
	{
	}
...
public:
	...
	void Subscribe(Observer* pObserver)
	{
		...
		{
			ObserverInfo observerInfo = { pObserver, FALSE };
	...
	//-----------------------------------------------------------------------
	// Summary: Notifies all subscribed observers about an event determined
	//          by observer's method to be called for each subscribed observer.
	// Parameters:
...
private:
	...
	void OnAfterNotify()
	{
		if (0 == --m_nNotificationDepth)
		{
			...
			{
				m_Observers.RemoveAt(indices[i - 1]);
			}
		}
...
//-----------------------------------------------------------------------
#define XTP_DYNAMIC_DOWNCAST_REMOTE(className, object) \
	(className*)XTPDynamicDownCastRemote(RUNTIME_CLASS(className), object)
